...
void LED_PC13_Turn(void)//实现按一下熄灭，再按一下点亮该如何实现，就是按键按下，LED的状态取反，实现电平翻转，一般用于输出模式下
{
	if(GPIO_ReadOutputDataBit(LED_PC13_PORT,LED_PC13_GPIO_PIN) == 0)//如果PA0的输出寄存器为0
	{
		GPIO_SetBits(LED_PC13_PORT,LED_PC13_GPIO_PIN);//将PA0置1
	}else
	{
		GPIO_ResetBits(LED_PC13_PORT,LED_PC13_GPIO_PIN);
	}
